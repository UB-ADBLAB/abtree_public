...
quick_descend_path:
	...
	/* We have only have tuple that is possibly overlapping with our range. */
	if (minoff == maxoff)
	{
		...
		else
		{
			uint64	agg;
			/* 
			 * Need to set up two things: the child page target blkno, a cutoff
			 * value and the expected high key in the child page. See the
			 * comments below.
			 */
			blkno = ABTreeTupleGetDownLink(itup);
			...
			/*
			 * At this point, we have to make descend into the child page. At
			 * the same time, the child page could be split, so we need to
			 * check if we have a matching key once we're there. If not,
			 * we go with the regular path descend path.
			 */
			buf = _abt_relandgetbuf(rel, buf, blkno, ABT_READ);
