		 ...
		 ExecScanRecheckMtd recheckMtd)
{
	...
	for (;;)
	{
		...
		else
		{
			...
			/* 
			 * A special case if we are doing index sample scan, we'll
			 * be interested if this tuple gets rejected and we'll have to
			 * retry.
			 */
			if (IsA(node, IndexScanState))
			{
				IndexScan *plan = (IndexScan *) node->ps.plan;
				if (plan->is_sample_scan)
					index_sample_scan_new_sample_rejected(
						&((IndexScanState *) node)->iss_IndexSampleScanState);
			}
			else if (IsA(node, IndexOnlyScanState))
			{
				IndexOnlyScan *plan = (IndexOnlyScan *) node->ps.plan;
				if (plan->is_sample_scan)
					index_sample_scan_new_sample_rejected(
						&((IndexOnlyScanState *) node)
							->ioss_IndexSampleScanState);
			}
		}
