 ...
 *		IndexNextSample			sample next tuple using index
...
static TupleTableSlot *IndexNextSample(IndexScanState *node);
 ...
 *		IndexNextSample
 *
 *		Sample a tuple from the IndexScan node's currentRelation
 *		using the index specified in the IndexScanState information.
 * ----------------------------------------------------------------
 */
static TupleTableSlot *
IndexNextSample(IndexScanState *node)
{
	EState	   *estate;
	ExprContext *econtext;
	ScanDirection direction;
	...
	if (index_sample_scan_no_more_samples(&node->iss_IndexSampleScanState))
	{
		/* no more samples to return for this scan. */
		return NULL;
	}
	...
	for (;;)
	{
		...
		random_number = index_sample_scan_next_random_number(
			&node->iss_IndexSampleScanState);
		res = index_samplenext_slot(scandesc, random_number, slot);
		...
		index_sample_scan_got_new_sample(&node->iss_IndexSampleScanState);
		return slot;
	}
...
ExecIndexScan(PlanState *pstate)
{
	...
	if (((IndexScan *) node->ss.ps.plan)->is_sample_scan)
		return ExecScan(&node->ss,
						(ExecScanAccessMtd) IndexNextSample,
...
ExecReScanIndexScan(IndexScanState *node)
{
	...
	/*
	 * Restart the sample scan if needed.
	 */
	if (((IndexScan *) node->ss.ps.plan)->is_sample_scan)
	{
...
ExecInitIndexScan(IndexScan *node, EState *estate, int eflags)
{
	...
	if (node->is_sample_scan)
	{
		...
		index_sample_scan_init(&indexstate->iss_IndexSampleScanState,
							   node->repeatable_expr,
							   (PlanState *) indexstate,
							   node->sample_size,
							   node->sample_size_expr);
		index_sample_scan_start(&indexstate->iss_IndexSampleScanState,
