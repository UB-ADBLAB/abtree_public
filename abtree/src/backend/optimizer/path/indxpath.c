...
typedef enum
{
	...
	ST_SAMPLESCAN,				/* must support amsamplescan */
	ST_ANYSCAN					/* 
								 * either index scan or bitmap scan is okay,
								 * XXX this is not a good name since we've added
								 * a third scan type 
								 **/
} ScanTypeControl;
...
static void get_index_paths(PlannerInfo *root, RelOptInfo *rel,
							IndexOptInfo *index, TableSampleClause *tsc,
							IndexClauseSet *clauses, List **bitindexpaths);
 ...
 */
void
create_index_paths(PlannerInfo *root, RelOptInfo *rel, TableSampleClause *tsc)
{
	List	   *indexpaths;
	List	   *bitindexpaths;
	List	   *bitjoinpaths;
	...
	foreach(lc, rel->indexlist)
	{
		 ...
		 */
		get_index_paths(root, rel, index, tsc, &rclauseset, &bitindexpaths);
	
		/* 
		 * If there's a tablesample clause that requires index path,
		 * ignore that. Note that, the original PG doesn't generate any
		 * index paths if there's a tablesample clause because that's a
		 * SampleScan path.
		 */
		if (tsc) continue;
					 ...
					 List **considered_relids)
{
	...
	get_index_paths(root, rel, index, NULL, &clauseset, bitindexpaths);
				...
				List **bitindexpaths)
{
	...
	foreach(lc, indexpaths)
	{
		...
		if (ipath->is_sample_scan)
		{
			Assert(index->amhassampletuple);
				  ...
				  TableSampleClause *tsc)
{
	...
	Assert(!tsc || scantype == ST_SAMPLESCAN);
	...
	switch (scantype)
	{
		...
		case ST_BITMAPSCAN:
			...
			break;
		case ST_SAMPLESCAN:
			if (!index->amhassampletuple)
				return NIL;
			break;
		case ST_ANYSCAN:
			/* ST_INDEXSCAN or ST_BITMAPSCAN are OK */
			break;
	}
	 ...
	 * if we are only trying to build bitmap indexscans or sample scan, nor if
	 * we have to assume the scan is unordered.
	 */
	pathkeys_possibly_useful = (scantype != ST_BITMAPSCAN &&
								scantype != ST_SAMPLESCAN &&
	 ...
	 * predicate, OR an index-only scan is possible, or we are building a
	 * sample scan.
	 */
	if (index_clauses != NIL || useful_pathkeys != NIL || useful_predicate ||
		index_only_scan || scantype == ST_SAMPLESCAN)
	{
		 ...
		 * parallel index scan for bitmap index scans or sample scan.
		 */
		if (index->amcanparallel &&
			rel->consider_parallel && outer_relids == NULL &&
			scantype != ST_BITMAPSCAN && scantype != ST_SAMPLESCAN)
		{
			ipath = create_index_path(root, index,
									  index_clauses,
									  orderbyclauses,
	 ...
	 */
	if (index_is_ordered && pathkeys_possibly_useful && scantype != ST_SAMPLESCAN)
	{
		index_pathkeys = build_index_pathkeys(root, index,
