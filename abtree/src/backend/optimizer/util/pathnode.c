 ...
 * 'tsc' is not NULL if we will use this index to perform a sample scan
 *
				  ...
				  TableSampleClause *tsc)
{
	...
	/* 
	 * This is not an ideal place to check whether we have incorrectly add an
	 * orderby clause to this path if it is a index sample scan. But let's be
	 * cautious.
	 */
	if (tsc && list_length(indexorderbys) != 0)
	{
		ereport(ERROR,
				(errcode(ERRCODE_INTERNAL_ERROR),
				 errmsg("index sample scan cannot have an orderby clause")));
	}
	...
	if (tsc)
	{
		...
		Assert(tsc->tsmhandler == F_TSM_SWR_HANDLER);
			...
			!((Const *) num_rows_node)->constisnull)
		{
			...
			pathnode->sample_size_expr = NULL;
		} else {
			// In a cached plan, we assume the cost is at least one visit
			num_rows = 1;
			pathnode->sample_size_expr = (Expr *) num_rows_node;
		}
		...
		if (num_rows < 0)
		{
			ereport(ERROR,
					(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
					 errmsg("negative row count %ld in TABLESAMPLE swr",
							num_rows)));
		}

		pathnode->is_sample_scan = true;
		pathnode->sample_size = (uint64) num_rows;
		pathnode->repeatable_expr = tsc->repeatable;
	}
	else
	{
		pathnode->is_sample_scan = false;
		pathnode->sample_size = 0;
		pathnode->repeatable_expr = NULL;
	}
