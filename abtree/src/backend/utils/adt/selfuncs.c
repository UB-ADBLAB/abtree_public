			   ...
			   double *indexPages)
{
	...
	if (path->is_sample_scan)
	{
		double num_tuples_in_rel;

		/* 
		 * Sample scan has a different cost: redescend on the tree for
		 * every tuple. This is a conservative estimate where each
		 * access to the leaf page is an I/O
		 */
		memset(&costs, 0, sizeof(costs));
		costs.numIndexPages = path->sample_size;
		costs.numIndexTuples = path->sample_size;
		genericcostestimate(root, path, loop_count, &costs);
		
		costs.indexTotalCost += (index->tree_height + 1) * cpu_operator_cost *
			path->sample_size;
		
		num_tuples_in_rel= path->indexinfo->rel->tuples;

		*indexStartupCost = costs.indexStartupCost;
		*indexTotalCost = costs.indexTotalCost;
		*indexSelectivity =
			(1.0 - pow(1.0 - 1.0 / num_tuples_in_rel, path->sample_size));
		*indexCorrelation = 0;
		*indexPages = path->sample_size;
