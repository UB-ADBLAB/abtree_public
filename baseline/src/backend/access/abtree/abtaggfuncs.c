...
/*Datum
abt_count_int8_init_fn(PG_FUNCTION_ARGS)
{
	PG_RETURN_INT64(0);
}

Datum
abt_count_int8_map_fn(PG_FUNCTION_ARGS)
{
	int64 n;	
	IndexTuple itup = (IndexTuple) PG_GETARG_POINTER(0);

	if (ABTreeTupleIsPosting(itup)) {
		n = (int64) ABTreeTupleGetNPosting(itup);
	} else {
		n = 1;
	}
