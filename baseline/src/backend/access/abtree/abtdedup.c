 ...
 * 
 * In case something gets deleted, the summation of the aggregation values of
 * the deleted items are added to *p_deleted_sum. If is_first is true,
 * *p_deleted_sum is not required to be initialized.  Returns true if anything
 * is deleted.
 */
bool
_abt_dedup_one_page(Relation rel, Buffer buf, Relation heapRel,
				   ...
				   Datum *p_deleted_sum, bool is_first)
{
	OffsetNumber offnum,
				minoff,
				maxoff;
	...
	state->save_agg = agg_info->leaf_has_agg;  /* also need to copy over the agg values */
	state->nmaxitems = 0;
	...
	if (RelationNeedsWAL(rel))
	{
		...
		{
			/* Needed by ABTMaxItemSize() to set maxpostingsize. */
			int16 ext_space = (int16) agg_info->extra_space_when_tid_present;
			XLogRegisterData((char *) &ext_space, sizeof(int16));
		}
						...
						OffsetNumber baseoff, ABTCachedAggInfo agg_info)
{
	...
	Assert(!state->save_agg || agg_info->leaf_has_agg);
	...
	if (IndexTupleSize(base) <= state->maxpostingsize)
	{
		...
		if (!ABTreeTupleIsPosting(base))
		{
			...
			if (agg_info->leaf_has_agg)
			{
				state->basetupsize -= agg_info->extra_space_when_tid_missing;
			}
			if (state->save_agg)
			{
				memcpy(state->aggs,
		...
		else
		{
			...
			if (state->save_agg)
			{
				memcpy(state->aggs, ABTreePostingTupleGetAggPtr(base, agg_info),
					 ...
					 ABTCachedAggInfo agg_info)
{
	...
	if (state->save_agg)
	{
		...
		Assert(state->nhtids + nhtids <
			   ABTDedupMaxNumberOfTIDs(state, agg_info));
						  ...
						  ABTCachedAggInfo agg_info)
{
	...
	else
	{
		...
		if (state->save_agg)
		{
			memcpy(ABTreePostingTupleGetAggPtr(final, agg_info), state->aggs,
				state->nhtids * agg_info->agg_stride);
		}
				  ...
				  ABTCachedAggInfo agg_info)
{
	...
	else
	{
		...
		if (agg_info->leaf_has_agg)
		{
			newsize += agg_info->extra_space_when_tid_missing;
		}
	}
 ...
 *
 * Also need to swap the agg if agg_info->leaf_has_agg == true.
 */
IndexTuple
