...
static ABTStack _abt_search_insert(Relation rel, ABTInsertState insertstate,
								   bool xlock_path);
...
static void _abt_stepright(Relation rel, ABTInsertState insertstate, ABTStack stack);
static void _abt_insertonpg(Relation rel, ABTScanInsert itup_key,
						   Buffer buf,
						   Buffer cbuf,
						   ABTStack stack, /* parent stack */
						   ABTStack	cur_stack,
						   IndexTuple itup,
						   Size itemsz,
...
static inline bool _abt_pgaddtup(Page page, Size itemsize, IndexTuple itup,
								...
								ABTCachedAggInfo agg_info);
static bool _abt_vacuum_one_page(Relation rel, Buffer buffer, Relation heapRel,
								 ABTCachedAggInfo agg_info,
								 Datum *p_deleted_sum,
								 bool is_first);
static void _abt_fix_path_aggregation_item(Relation rel, ABTStack stack,
										   Buffer leaf_buf, IndexTuple item,
										   ABTCachedAggInfo agg_info,
										   bool is_add);
static void _abt_fix_path_aggregation_datum(Relation rel, ABTStack stack,
											Buffer leaf_buf,
											Datum datum,
											ABTCachedAggInfo agg_info,
											bool is_add);
			 ...
			 ABTCachedAggInfo agg_info)
{
	...
	bool		xlock_path = (checkUnique != UNIQUE_CHECK_EXISTING);
...
search:
	 ...
	 */
	stack = _abt_search_insert(rel, &insertstate, xlock_path);
	...
	if (checkUnique != UNIQUE_CHECK_EXISTING)
	{
		...
		leaf_stack.abts_blkno = BufferGetBlockNumber(insertstate.buf);
		leaf_stack.abts_offset = newitemoff;
		leaf_stack.abts_buf = insertstate.buf;
		leaf_stack.abts_parent = stack;
		/* 
		...
		_abt_insertonpg(rel, itup_key, insertstate.buf, InvalidBuffer,
						stack, &leaf_stack, itup, insertstate.itemsz,
						newitemoff, insertstate.postingoff, false, agg_info,
					    /*previtem_newagg = */Int16GetDatum(0),
						/*leafitem_in_left_subtree = */true);

		if (leaf_stack.abts_parent != stack)
		{
			Assert(!stack);
			stack = leaf_stack.abts_parent;
		}
	
		/* 
		 * The leaf might have been split so use the updated buffer in the
		 * leaf stack frame. 
		 */
		_abt_fix_path_aggregation_item(rel, stack, leaf_stack.abts_buf, itup,
									   agg_info, true);
		_abt_relbuf(rel, leaf_stack.abts_buf);
	}
	else
	{
		/* just release the buffer. The path is not x-locked. */
		_abt_relbuf(rel, insertstate.buf);
	}

	/* 
	 * be tidy and also release all the buffers in the stack.
	 */
	if (stack)
		_abt_freestack(rel, stack);
	pfree(itup_key);
...
/*
 *	_abt_search_insert() -- _abt_search() wrapper for inserts
 *
 * Search the tree for a particular scankey, or more precisely for the first
 * leaf page it could be on. We can't use the fastpath optimization in
 * aggregate B-tree because we'll always need the stack of the path.
 *
 * Another consideration is that we must lock the fast root if xlock_path ==
 * true, because otherwise, we can't unlock the concurrently split fast root
 * and try to find the current fast root without losing the consistency
 * guarantee.
 *
 */
static ABTStack
_abt_search_insert(Relation rel, ABTInsertState insertstate, bool xlock_path)
{

	/* Cannot use optimization -- descend tree, return proper descent stack */
	return _abt_search(rel, insertstate->itup_key, &insertstate->buf, ABT_WRITE,
					  NULL, xlock_path,
					  /* reject_non_fastroot = */xlock_path);
}
				  ...
				  ABTCachedAggInfo agg_info)
{
	...
	
	{
		...
		if (checkingunique)
		{
			...
			for (;;)
			{
				...
				elog(ERROR, "we shouln'd have to step right with a xlocked path");

				_abt_stepright(rel, insertstate, stack);
		...
		if (PageGetFreeSpace(page) < insertstate->itemsz)
		{
			...
			{
				deleted_sum_is_set |= _abt_vacuum_one_page(rel,
					insertstate->buf, heapRel, agg_info, &deleted_sum,
					!deleted_sum_is_set);
				insertstate->bounds_valid = false;
				...
				PageGetFreeSpace(page) < insertstate->itemsz)
			{
				deleted_sum_is_set |= 
					_abt_dedup_one_page(rel, insertstate->buf, heapRel,
										...
										&deleted_sum, !deleted_sum_is_set);
				insertstate->bounds_valid = false;
			}
		}
	}
	...
	if (insertstate->postingoff == -1)
	{
		 ...
		 */
		deleted_sum_is_set |=
			_abt_vacuum_one_page(rel, insertstate->buf, heapRel, agg_info,
				&deleted_sum, !deleted_sum_is_set);
	...
	/* XXX use the x-locked stack to propagate the deleted sum */
	if (deleted_sum_is_set)
		_abt_fix_path_aggregation_datum(rel, stack, insertstate->buf,
										deleted_sum, agg_info,
										/* is_add = */false);
...
_abt_stepright(Relation rel, ABTInsertState insertstate, ABTStack stack)
{
	...
	for (;;)
	{
		...
		if (ABT_P_INCOMPLETE_SPLIT(lpageop))
		{
			ereport(ERROR,
				(errcode(ERRCODE_INTERNAL_ERROR),
				 errmsg("We shound't see any incomplete split in baseline "
					    "because we x-lock the entire path.")));
			/* _abt_finish_split(rel, rbuf, stack); */
			rbuf = InvalidBuffer;
			continue;
		}
 ...
 *
 *		stack: parent stack of buf
 *
 *		cur_stack: the stack frame that points to buf, will be updated if a
 *		split happens and the pivot tuple that points to the subtree in which
 *		the new leaf item initially inserted is moved to the (new) right page
 *
 *		previtem_newagg: the old pivot tuple's aggregation value. It will not
 *		be touched if buf is leaf.
 *
 *		leafitem_in_left_subtree: indicates whether the new leaf item initially
 *		inserted is in the left subtree (pointed by newitemoff - 1), or in the
 *		right subtree (pointed by newitem). It will not be touched if buf is
 *		leaf.
 *
 *----------
 */
			   ...
			   bool leafitem_in_left_subtree)
{
	...
	{
		bool		is_leaf = ABT_P_ISLEAF(lpageop);
		bool		is_root = ABT_P_ISROOT(lpageop);
		bool		is_only = ABT_P_LEFTMOST(lpageop) && ABT_P_RIGHTMOST(lpageop);
		bool		is_rightmost = ABT_P_RIGHTMOST(lpageop);
		Buffer		rbuf;
		bool		newitemonleft;
		OffsetNumber firstrightoff;
		bool		leafitem_subtree_moved_to_right_page;
		OffsetNumber leafitem_subtree_offset;
		...
		rbuf = _abt_split(rel, itup_key, buf, cbuf, newitemoff, itemsz, itup,
						 ...
						 previtem_newagg, &newitemonleft, &firstrightoff);
		/* 
		 * cbuf points to the left subtree, which is no longer needed in
		 * the stack.
		 */
		if (!is_leaf && !leafitem_in_left_subtree)
		...
		/* 
		 * Determine whether the subtree containing the new leaf item initially
		 * inserted is now moved to the right page and its offset on the 
		 * original page.
		 */
		Assert(cur_stack->abts_buf == buf);
		if (is_leaf)
		{
			if (postingoff != 0)
			{
				/* 
				 * This involves a posting list split, meaning our new item
				 * is now inside the original posting tuple.
				 */
				leafitem_subtree_offset = OffsetNumberPrev(newitemoff);
				leafitem_subtree_moved_to_right_page = false;
			}
			else
			{
				leafitem_subtree_moved_to_right_page = !newitemonleft;
				leafitem_subtree_offset = newitemoff;
			}
		}
		else
		{
			if (leafitem_in_left_subtree)
			{
				leafitem_subtree_offset = OffsetNumberPrev(newitemoff);
				leafitem_subtree_moved_to_right_page =
					leafitem_subtree_offset >= firstrightoff;
			}
			else
			{
				leafitem_subtree_offset = newitemoff;
		...
		/* 
		 * Now fix the offset of the subtree that contains the new leaf item
		 * initially inserted if it's on the right page. And also update
		 * the current stack frame if applicable.
		 */ 
		if (leafitem_subtree_moved_to_right_page)
		{
			/* 
			 * This is correct because we are looking at 1) the new item itself
			 * (incl. the case of a pivot tuple that links to the new right
			 * child); 2) or one item prior to that, which can be either the
			 * posting list that contains the original new item, or the pivot
			 * tuple that links to the old left child that was just split.
			 *
			 * In any case, we can't be looking at some item on the original
			 * page that comes after the new item to be inserted to this page.
			 * Hence, we don't have to consider adding one because the new item
			 * gets inserted on the right page.
			 */
			leafitem_subtree_offset = leafitem_subtree_offset - firstrightoff
				+ (is_rightmost ? ABTP_HIKEY : ABTP_FIRSTKEY);
			
			cur_stack->abts_blkno = BufferGetBlockNumber(rbuf);
			cur_stack->abts_offset = leafitem_subtree_offset;
			cur_stack->abts_buf = rbuf;
		}
		else
		{
			/* 
			 * The offset can still change even if it stays on the original
			 * page in two cases:
			 * 1) The current page in the leaf page and there is a posting
			 * list split. 
			 * 2) The current page was the rightmost page on its level, and
			 * thus all the offsets are shifted to right by 1.
			 */
			if (is_rightmost)
			{
				leafitem_subtree_offset =
					OffsetNumberNext(leafitem_subtree_offset);
			}
			cur_stack->abts_offset = leafitem_subtree_offset;
		}
		 ...
		 * page.
		 *----------
		 * Note: _abt_insert_parent() only excludes the value of itup if
		 * is_leaf == true. This is necessary to avoid double accounting of the
		 * new item when we fix the aggregation on the path in _abt_doinsert().
		 *
		 * XXX this is a temporary solution that works for the baseline only.
		 * If there's concurrent split, and some other backends try to finish
		 * an incomplete split, it won't have any information about the new
		 * tuple.
		 *
		 * TODO solution: store the uncommitted aggregtaion value in the high
		 * key on leaf level and update upon transaction commit.
		 *
		 * Note the leafitem_in_left_subtree on the level above us (the parent
		 * page's level) indicates if the subtree containing the original
		 * new leaf item is the left or the right one.
		 */
		new_parentstack = _abt_insert_parent(
			rel, buf, rbuf, stack, is_root, is_only, agg_info, is_leaf,
			/*leafitem_in_left_subtree =*/!leafitem_subtree_moved_to_right_page,
			itup);
		...
		if (new_parentstack)
			cur_stack->abts_parent = new_parentstack;
	}
	else
	{
		...
		/* 
		 * Don't release cbuf (left subtree's root page buffer) here, unless
		 * the inserted new leaf item initially inserted is in the right
		 * subtree, in which case it is no longer needed to fix the path
		 * aggregation values and the caller, _abt_insert_parent() expects us
		 * to release the cbuf.
		 */
		if (!isleaf)
		{
			Assert(cur_stack->abts_offset = OffsetNumberPrev(newitemoff));
			if (!leafitem_in_left_subtree)
			{
				/* 
				 * Need to update the offset of the pivot tuple that points
				 * to the subtree that contains the new leaf item initially
				 * inserted to the right.
				 */
				cur_stack->abts_offset = newitemoff;
				_abt_relbuf(rel, cbuf);
			}
		}
		...
		/* 
		 * XXX don't release the buf here. Leave it to the caller.
 ...
 *		XXX will not release the buffer (baseline xlocks the entire path).
 *
 ...
 *		
 *		*newitem_on_left is set to true if the original new item is put on the
 *		left page, or false if it is put on the right page. Note that, in case
 *		of a posting list split, that is the location of the posting list tuple
 *		instead of the inserted new item (which has the max TID in the posting
 *		list).
 *
...
_abt_split(Relation rel, ABTScanInsert itup_key, Buffer buf, Buffer cbuf,
		  ...
		  bool *p_newitemonleft, OffsetNumber *p_firstrightoff)
{
	Buffer		rbuf;
	Page		origpage;
	Page		leftpage,
	...
	/* Pass the newitemonleft and firstrightoff info to the caller. */
	*p_newitemonleft = newitemonleft;
	*p_firstrightoff = firstrightoff;
	...
	if (isleaf)
	{
		...
		/* Will also truncate away the aggregation value from the high key. */
	...
	/* release the child */
	/* 
	 * XXX don't release the child page here: the baseline xlocks the 
	 * entire path. Our caller should decide whether the cbuf should be
	 * released or not.
	 */
	/*if (!isleaf)
		_abt_relbuf(rel, cbuf); */
 ...
 * still hold write locks on. We also have x-locked the entire path that
 * leads to buf in the tree, except for the case where we may have locked
 * a split root/fastroot when at the beginning of the search.
 *
				  ...
				  IndexTuple orignewitem)
{
	...
	/* compute the sum of the aggregation values on both pages */
	lagg = _abt_accum_page_value(agg_info, buf);
	ragg = _abt_accum_page_value(agg_info, rbuf);
	
	/* exclude the newitem's aggregation value from the page sum */
	if (is_leaf)
	{
		_abt_exclude_index_tuple_value(agg_info,
			leafitem_in_left_subtree ? &lagg : &ragg,
			orignewitem, /* is_leaf = */true);
	}
	...
	if (is_root)
	{
		...
		return_stack->abts_buf = rootbuf;
		return_stack->abts_parent = NULL;
	
		/* 
		 * We only need to keep the pin and lock on the buffer in the
		 * corresponding subtree of which our new leaf item initially inserted.
		 */
		if (leafitem_in_left_subtree)
		{
			return_stack->abts_offset = ABTP_HIKEY;
			_abt_relbuf(rel, rbuf);
		}
		else
		{
			return_stack->abts_offset = ABTP_FIRSTKEY;
			_abt_relbuf(rel, buf);
		}
	}
	else
	{
		...
		/*
		 * Because we x-lock the path, just get that from the stack unless this
		 * is the left most page above the fast root level.
		 *
		 * XXX Re-find and write lock the parent of buf.
		 *
		 ...
		 */
		/*pbuf = _abt_getstackbuf(rel, stack, bknum); */
		if (stack == &fakestack)
		{
			pbuf = _abt_getstackbuf(rel, stack, bknum);
			return_stack = palloc(sizeof(ABTStackData));
			return_stack ->abts_blkno = stack->abts_blkno;
			return_stack ->abts_offset = stack->abts_offset;
			return_stack->abts_buf = pbuf;
			return_stack->abts_parent = NULL;
			stack = return_stack;
		}
		else
		 ...
		 *
		 * We no long need the right page buffer at this point unless it is
		 * where our new leaf item initially inserted is.
		 */
		if (leafitem_in_left_subtree)
			_abt_relbuf(rel, rbuf);
		...
		_abt_insertonpg(rel, NULL, pbuf, buf, stack->abts_parent,
						...
						lagg, leafitem_in_left_subtree);

		/* 
		 * insertonpg() should release the left child if it's not needed
		 * after this point (!leafitem_in_left_subtree). 
		 */
...
_abt_finish_split(Relation rel, Buffer lbuf, ABTStack stack)
{
	...
	elog(ERROR, "shouldn't reach here in baseline");
	/* _abt_insert_parent(rel, lbuf, rbuf, stack, was_root, was_only); */
	/* XXX suppress unused warnings */
	(void) was_only;
	(void) (was_root);
}
		...
		if (ABT_P_INCOMPLETE_SPLIT(opaque))
		{
			ereport(ERROR,
				(errcode(ERRCODE_INTERNAL_ERROR),
				 errmsg("We shound't see any incomplete split in baseline "
					    "because we x-lock the entire path.")));
			/*_abt_finish_split(rel, buf, stack->abts_parent); */
			continue;
		}
			 ...
			 ABTCachedAggInfo agg_info, Datum lagg, Datum ragg)
{
	...
	/* update right_item_sz as it should increases by the size of agg_type */
 ...
 * 
 * The summation of the aggregation values of the deleted items are added
 * to *p_deleted_sum. If is_first is true, *p_deleted_sum is not required to
 * be initialized.
 *
 * Returns true if anything is deleted.
 */
static bool
_abt_vacuum_one_page(Relation rel, Buffer buffer, Relation heapRel,
					ABTCachedAggInfo agg_info, Datum *p_deleted_sum,
					bool is_first)
{
		 ...
		 offnum = OffsetNumberNext(offnum))
	{
		...
		if (ItemIdIsDead(itemId))
		{
			...
			_abt_accum_index_tuple_value(agg_info, p_deleted_sum, itup,
				/* is_leaf = */true,
				/* is_first = */is_first && ndeletable == 1);
		}
	}
...
static void
_abt_fix_path_aggregation_item(Relation rel, ABTStack stack, Buffer leaf_buf,
							   IndexTuple item, ABTCachedAggInfo agg_info,
							   bool is_add)
{
	Pointer		agg_ptr;
	Datum		val;
	
	Assert(!ABTreeTupleIsPivot(item));
	Assert(!ABTreeTupleIsPosting(item));

	if (agg_info->leaf_has_agg)
	{
		agg_ptr = ABTreeNonPivotTupleGetAggPtr(item, agg_info);
		val = fetch_att(agg_ptr, agg_info->agg_byval, agg_info->agg_typlen);
	}
	else
	{
		val = agg_info->agg_map_val;
	}

	_abt_fix_path_aggregation_datum(rel, stack, leaf_buf, val, agg_info,
									is_add);
}

/*
 * XXX Stack contains a x-locked path for which we will add/subtract datum
 * from all the pivot tuples indicated by the offset in the stack.
 *
 * XXX must also do add in top-down order, and subtract in bottom up order.
 */
static void
_abt_fix_path_aggregation_datum(Relation rel, ABTStack stack, Buffer leaf_buf,
								Datum datum, ABTCachedAggInfo agg_info,
								bool is_add)
{
#ifdef USE_ASSERT_CHECKING
	/* 
	 * prev_blkno stores the block number of the child page if we are doing
	 * bottom-up, or the down link in the parent page if we are doing top-down.
	 * It will be matched against the parent down link, (or the child page
	 * block number, resp.).
	 */
	...
	/* We need to do it top-down instead of bottom-up if adding a number. */
	if (is_add)
	{
		stack = _abt_inverse_stack(stack);
	}
	else
	{
#ifdef USE_ASSERT_CHECKING
		prev_blkno = BufferGetBlockNumber(leaf_buf);
#endif
	}
	...
	while (cur_stack)
	{
		...
		buf = cur_stack->abts_buf;
		Assert(BufferIsValid(buf));
		page = BufferGetPage(buf);
		itemid = PageGetItemId(page, cur_stack->abts_offset);
		itup = (IndexTuple) PageGetItem(page, itemid);
		
		/* 
		 * If we are fixing it bottom-up, match the down link before we
		 * continue.
		 */
		Assert(is_add || ABTreeTupleGetDownLink(itup) == prev_blkno);
#ifdef USE_ASSERT_CHECKING	
		if (is_add)
		{
			prev_blkno = ABTreeTupleGetDownLink(itup);
		}
		else
		{
			prev_blkno = cur_stack->abts_blkno;
		}
#endif

		agg_ptr = ABTreePivotTupleGetAggPtr(itup, agg_info);
		old_agg = fetch_att(agg_ptr, agg_info->agg_byval, agg_info->agg_typlen);

		new_agg = FunctionCall2(
			is_add ? &agg_info->agg_add_fn : &agg_info->agg_sub_fn,
			old_agg,
			datum);
		...
		if (agg_info->agg_byval)
		{
			store_att_byval(agg_ptr, new_agg, agg_info->agg_typlen);
		}
		else
		{
			memcpy(agg_ptr, DatumGetPointer(new_agg), agg_info->agg_typlen);
		}
		MarkBufferDirty(buf);

		if (RelationNeedsWAL(rel))
		{
			xl_abtree_fix_aggregation_value xlrec;
			XLogRecPtr		recptr;

			ABTSetXLogAggOpNumber(xlrec.tag, XLOG_ABTREE_AGG_OP_FIX_AGG);
			xlrec.offset = agg_ptr - (char *) page;

			XLogBeginInsert();
			XLogRegisterData((char *) &xlrec, SizeOfABTreeFixAggregationValue);
	
			/* store the updated value in blk 0. */
			XLogRegisterBuffer(0, buf, REGBUF_STANDARD);
			XLogRegisterBufData(0, agg_ptr, agg_info->agg_typlen);
		...
		if (!agg_info->agg_byval)
			pfree(DatumGetPointer(new_agg));

		cur_stack = cur_stack->abts_parent;
	
		/* 
		 * If we are fixing it top-down, match the down link after we move
		 * to one level lower. 
		 */
		Assert(!is_add || ((!cur_stack && BufferIsValid(leaf_buf) &&
							prev_blkno == BufferGetBlockNumber(leaf_buf)) ||
						   (cur_stack && cur_stack->abts_blkno == prev_blkno)));
	}
	
	/* Restore the stack. */
	if (is_add)
		_abt_inverse_stack(stack);
}
