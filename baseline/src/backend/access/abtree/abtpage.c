 ...
 */
Buffer
_abt_getroot(Relation rel, int access, bool xlock, bool reject_non_fastroot)
{
	Buffer		metabuf;
	Buffer		rootbuf;
	Page		rootpage;
	...
	if ((metad = _abt_get_cached_metapage(rel)) != NULL)
	{
		...
		rootbuf = _abt_getbuf(rel, rootblkno, xlock ? ABT_WRITE : ABT_READ);
...
retry:
	...
	else
	{
		...
		for (;;)
		{
			rootbuf = _abt_relandgetbuf(rel, rootbuf, rootblkno,
				xlock ? ABT_WRITE : ABT_READ);
			rootpage = BufferGetPage(rootbuf);
			rootopaque = (ABTPageOpaque) PageGetSpecialPointer(rootpage);
...
_abt_pagedel(Relation rel, Buffer leafbuf, TransactionId *oldestBtpoXact)
{
	...
	for (;;)
	{
		...
		if (!ABT_P_ISHALFDEAD(opaque))
		{
			...
			if (!_abt_mark_page_halfdead(rel, leafbuf, stack))
			{
				_abt_relbuf(rel, leafbuf);
				return ndeleted;
			}
						...
						BlockNumber *topparent, BlockNumber *topparentrightsib)
{
	 ...
	 *
	 * XXX in the baseline, because we x-lock the entire path for an insertion,
	 * it is not possible to have incomplete split on the parent.
	 */
	pbuf = _abt_getstackbuf(rel, stack, child);
