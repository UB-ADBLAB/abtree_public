		 ...
		 IndexInfo *indexInfo)
{
	...
	if (agg_info->leaf_has_agg)
	{
		...
		itup = index_form_tuple_with_extra(RelationGetDescr(rel),
			values, isnull, agg_info->extra_space_when_tid_missing, 'm');
	...
	{
		/* The aggregation value is constant and will be used inline. */
...
#endif
		so->prefix_sums = palloc(max_num_items * sizeof(Datum));
...
abtbeginscan(Relation rel, int nkeys, int norderbys)
{
	...
	so->agg_info = NULL;		/* will be initialized in abtsampletuple() */
	so->inskey_low = NULL;
	so->inskey_high = NULL;
