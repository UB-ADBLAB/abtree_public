...
static OffsetNumber _abt_binsrch(Relation rel, ABTScanInsert key, Buffer buf);
static int	_abt_binsrch_posting(ABTScanInsert key, Page page,
								OffsetNumber offnum);
...
/*
 *	_abt_search() -- Search the tree for a particular scankey,
 ...
 */
ABTStack
_abt_search(Relation rel, ABTScanInsert key, Buffer *bufP, int access,
		   Snapshot snapshot, bool xlock_path, bool reject_non_fastroot)
{
	ABTStack		stack_in = NULL;
	int			page_access = 
		(access == ABT_WRITE && xlock_path) ? ABT_WRITE : ABT_READ;

	/* Get the root page to start with */
	*bufP = _abt_getroot(rel, access, /* xlock =*/page_access == ABT_WRITE,
						 reject_non_fastroot); 
	...
	for (;;)
	{
		...
		*bufP = _abt_moveright(rel, key, *bufP, (access == ABT_WRITE), stack_in,
		...
		if (page_access == ABT_WRITE)
		{
			new_stack->abts_buf = *bufP;
		}
		else
		{
			new_stack->abts_buf = InvalidBuffer;
		}
		...
		if (xlock_path)
		{
			/* just acquire the write lock on the child */
			*bufP = _abt_getbuf(rel, blkno, page_access);
		}
		else
		{
			/* drop the read lock on the parent, acquire one on the child */
	 ...
	 */
	if (access == ABT_WRITE && page_access == ABT_READ)
	{
	...
	for (;;)
	{
		...
		if (forupdate && ABT_P_INCOMPLETE_SPLIT(opaque))
		{
			...
			if (ABT_P_INCOMPLETE_SPLIT(opaque))
				_abt_finish_split(rel, buf, stack);
			else
				_abt_relbuf(rel, buf);
				 ...
				 Snapshot snapshot)
{
	 ...
	 */
	if (level == 0)
		buf = _abt_getroot(rel, ABT_READ, false, false);
	else
...
_abt_sample(IndexScanDesc scan, double random_number)
{
	...
	Page			page;
	ABTPageOpaque	opaque;
	Datum			*prefix_sums = so->prefix_sums;
	MemoryContext	prefix_sums_context = so->prefix_sums_context,
					old_context = NULL; /* suppress compiler warning */
	...
	Assert(agg_info && prefix_sums &&
		   (agg_info->agg_byval || prefix_sums_context));
	 ...
	 */
	buf = _abt_getroot(rel, ABT_READ, /* xlock =*/false,
					   /* reject_non_fastroot =*/true);
	if (!BufferIsValid(buf))
		ereport(ERROR,
				(errcode(ERRCODE_NO_DATA),
				 errmsg("can't sample an empty table")));
	page = BufferGetPage(buf);
	opaque = (ABTPageOpaque) PageGetSpecialPointer(page);
	for (;;)
	{
		bool	isleaf = ABT_P_ISLEAF(opaque);
		if (so->inskey_low)
		{
		...
		{
			/* 
			 * The range doesn't contain anything that matches our search key.
			 * This can only happen when we reach a leaf page through a single
			 * path that completely covers the key ranges.
			 *
			 * Note that this also includes the case where minoff happens to
			 * fall on 1 past the last key on the leaf page.
			 */
			Assert(isleaf);
			_abt_relbuf(rel, buf);
			return false;
		...
		{
			/* 
			 * Ok, we have at least two choices in the current page. Now we
			 * need to break the loop and make the random choice.
			 */
			break;
		}

		/* Otherwise, minoff is at the only choice we have on this page. */
		if (isleaf)
		{
			/* 
		...
		{
			/* 
			 * Descend through the only link we have now.
			 * Remember we have to use lock coupling here in the baseline, so
			 * that we don't have to deal with concurrent split.
			 */
			itemid = PageGetItemId(page, minoff);
			itup = (IndexTuple) PageGetItem(page, itemid);
			blkno = ABTreeTupleGetDownLink(itup);

			/* grab the read lock on the child page */
			cbuf = _abt_getbuf(rel, blkno, ABT_READ);
			/* release the lock on the parent */
			_abt_relbuf(rel, buf);
			/* and set up the page and opaque pointers */
			buf = cbuf;
			page = BufferGetPage(buf);
			opaque = (ABTPageOpaque) PageGetSpecialPointer(page);
		}
	...
	/* search for the lower bound and the upper bound */
	...
	{
		/* 
		 * Switch to the prefix sums context so that we can reset it after we
		 * are done with the prefix sums. This is only needed when the
		 * aggregation type is not passed by value, in which case the numerical
		 * function calls use palloc() to allocate new space for the results.
		 */
		MemoryContextReset(prefix_sums_context);
		old_context = MemoryContextSwitchTo(prefix_sums_context);
	}
	if (ABT_P_ISLEAF(opaque))
	{
		for (offnum = minoff; offnum <= maxoff; ++offnum)
		{
			bool		is_first = (num_items == 0);
			...
			if (ABTreeTupleIsPosting(itup))
			{
				...
				if (agg_info->leaf_has_agg)
				{
					...
					for (i = 0; i < n; ++i)
					{
						...
						prefix_sums[num_items] = _abt_accum_value_nofree(
							agg_info,
							is_first ? Int32GetDatum(0) :
									   prefix_sums[num_items - 1],
							fetch_att(agg_ptr, agg_info->agg_byval,
									  agg_info->agg_typlen),
							is_first);
	...
	else
	{
		for (offnum = minoff; offnum <= maxoff; ++offnum)
		{
			...
			prefix_sums[num_items] = _abt_accum_value_nofree(
				agg_info,
				is_first ? Int32GetDatum(0) : prefix_sums[num_items - 1],
				fetch_att(agg_ptr, agg_info->agg_byval, agg_info->agg_typlen),
				is_first);
			item_indexes[num_items] = ABT_OFFSET_GET_ITEMINDEX(offnum);
			++num_items;
		}
	...
	{
		Datum	total_sum = prefix_sums[num_items - 1];
		Datum	total_sum_float8;
		double	total_sum_float8_value;
		double	cutoff_float8;
		int		low,
				high,
				mid,
				cmpres;
	
		if (agg_info->agg_type_to_double_fn_initialized)
		{

			total_sum_float8 = FunctionCall1(&agg_info->agg_type_to_double_fn,
											 total_sum);
			total_sum_float8_value = (double) DatumGetFloat8(total_sum_float8);
		}
		else
		{
			total_sum_float8_value = (double) DatumGetFloat8(total_sum);
		}
		cutoff_float8 = total_sum_float8_value * random_number;
		if (!agg_info->agg_is_floating_point)
		{
			cutoff_float8 = floor(cutoff_float8);
		}
	
		if (agg_info->double_to_agg_type_fn_initialized)
		{
			cutoff = FunctionCall1(&agg_info->double_to_agg_type_fn,
								   Float8GetDatum((float8) cutoff_float8));
		}
		else
		{
			cutoff = Float8GetDatum(cutoff_float8);
		}
		...
		{
			mid = (low + high) >> 1;
			cmpres = DatumGetInt32(FunctionCall2(
	...
	if (ABT_P_ISLEAF(opaque))
	{
		...
		/* Be tidy. */
		if (agg_info->leaf_has_agg && !agg_info->agg_byval)
			pfree(DatumGetPointer(cutoff));
		goto done_sampling;
	}
	 ...
	 */
	if (cutoff_index != 0)
		_abt_exclude_value(agg_info, &cutoff, prefix_sums[cutoff_index - 1]);
	for (;;) {
		Datum acc;

		/* remember to use lock coupling when descending. */
		cbuf = _abt_getbuf(rel, blkno, ABT_READ);
		_abt_relbuf(rel, buf);
		buf = cbuf;
		page = BufferGetPage(buf);
		...
		if (ABT_P_ISLEAF(opaque))
		{
			...
			for (offnum = minoff; offnum <= maxoff; ++offnum)
			{
				...
				if (ABTreeTupleIsPosting(itup))
				{
					...
					if (agg_info->leaf_has_agg)
						agg_ptr = ABTreePostingTupleGetAggPtr(itup, agg_info);
					for (postingi = 0; postingi < n; ++postingi)
					{
						if (agg_info->leaf_has_agg)
						{
							_abt_accum_value(agg_info, &acc,
								fetch_att(agg_ptr,
										  agg_info->agg_byval,
										  agg_info->agg_typlen),
								/* is_first = */minoff == offnum &&
												postingi == 0);
							agg_ptr += agg_info->agg_stride;
						}
						else
						...
						if (cmpres > 0)
						{
							...
							chosen_leaf_item_posting_off = postingi;
							if (!agg_info->agg_byval)
			...
			/* 
			 * If we end up here, there must be dead tuples removed. Reject
			 * this sample run.
			 */
			_abt_relbuf(rel, buf);
			if (!agg_info->agg_byval)
		...
		else
		{
			...
			for (offnum = minoff; offnum <= maxoff; ++offnum)
			{
				...
				itup = (IndexTuple) PageGetItem(page, itemid);
				agg_ptr = ABTreePivotTupleGetAggPtr(itup, agg_info);
				new_acc = _abt_accum_value_nofree(
					agg_info,
					acc,
					fetch_att(agg_ptr,
							  agg_info->agg_byval,
				...
				{
					/* 
					 * Found it. Subtract ``acc'' from cutoff before we
					 * continue.
					 */
					if (minoff != offnum)
						_abt_exclude_value(agg_info, &cutoff, acc);
					if (!agg_info->agg_byval)
						pfree(DatumGetPointer(acc));
					acc = new_acc;
					blkno = ABTreeTupleGetDownLink(itup);
			...
			{
				/* 
				 * If we end up here, we are out of the range and have to
				 * reject this sample run. 
				 */
				_abt_relbuf(rel, buf);
				return false;
			}
