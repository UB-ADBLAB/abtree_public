		   ...
		   ABTSpool *btspool, ItemPointer self, Datum *values, bool *isnull)
{
	...
	{
		/* 
		 * Need to reserve some space after all the key and include columns
		 * for the aggregation value. We maxalign the extra space so that
		 * we can safely find the end of the keys when needed.
		 */
		tuplesort_putindextuplevalues_with_extra(
			btspool->sortstate, btspool->index,
			self, values, isnull, agg_info->extra_space_when_tid_missing,
			'm');
	}
	else
	{
			  ...
			  ABTCachedAggInfo agg_info)
{
	 ...
	 *
	 */
	if (unlikely((agg_info->leaf_has_agg ?
			(itupsz - agg_info->extra_space_when_tid_missing) :
	...
	if (agg_info->leaf_has_agg && isleaf)
	{
							   ...
							   ABTCachedAggInfo agg_info)
{
	...
	else
	{
		...
		postingtuple = _abt_form_posting(dstate->base,
										...
										dstate->nhtids,
										agg_info);
		/* Calculate posting list and aggregation array overhead */
...
_abt_load(ABTBuildState *buildstate, ABTWriteState *wstate)
{
	...
	else if (deduplicate)
	{
		...
		dstate->save_agg = false; /* we'll initialize them by ourselves */
