...
static void _abt_compute_pivot_tuple_offsets(ABTCachedAggInfo agg_info);
 ...
 */
ABTScanInsert
_abt_mkscankey(Relation rel, IndexTuple itup)
{
	ABTScanInsert key;
	ScanKey		skey;
	TupleDesc	itupdesc;
...
_abt_freestack(Relation rel, ABTStack stack)
{
	...
	while (stack != NULL)
	{
		...
		if (BufferIsValid(ostack->abts_buf))
		{
			_abt_relbuf(rel, ostack->abts_buf);
		}
		pfree(ostack);
	}
}
...
/*
 * Release all the buffers in the stack if they are currently pinned and/or
 * locked, without deallocating the stack.
 */
void
_abt_relbuf_stack(Relation rel, ABTStack stack)
{
	while (stack != NULL)
	{
		if (BufferIsValid(stack->abts_buf))
		{
			_abt_relbuf(rel, stack->abts_buf);
...
static void
_abt_compute_pivot_tuple_offsets(
	ABTCachedAggInfo agg_info)
{
	Size	ext_space_wo_tid;
	Size	ext_space_w_tid;

	ext_space_wo_tid = MAXALIGN(agg_info->agg_typlen);
	
	/* ItemPointerData can be short-aligned */
	ext_space_w_tid = MAXALIGN(SHORTALIGN(agg_info->agg_typlen) +
				   ...
				   ABTAggSupport agg_support)
{
	...
	agg_info->agg_stride = att_align_nominal(agg_type->typlen,
												 agg_type->typalign);
	Assert(agg_info->agg_stride <= MAXALIGN(agg_type->typlen));
	...
	_abt_compute_pivot_tuple_offsets(agg_info);
}
