				  ...
				  XLogReaderState *record)
{
	...
	if (posting)
	{
		if (data_len > SizeOfABtreeInsert)
		{
			 ...
			 * agg info if the leaf level has stored agg.
			 */
...
abtree_xlog_dedup(XLogReaderState *record)
{
	...
	if (data_len == SizeOfABtreeDedup + SizeOfABtreeMinimalAggInfo)
	{
		xl_abtree_minimal_agg_info	min_agg_info;
	...
	{
		/* needed by ABTMaxItemSize() */
		int16 ext_space;
		Assert(SizeOfABtreeDedup + sizeof(int16) == data_len);
		memcpy(&ext_space,
			  ((char *) xlrec) + SizeOfABtreeDedup,
			  sizeof(int16));
		agg_info->extra_space_when_tid_present = ext_space;
		agg_info->leaf_has_agg = false;
	}
	...
	if (XLogReadBufferForRedo(record, 0, &buf) == BLK_NEEDS_REDO)
	{
		...
		state->save_agg = agg_info->leaf_has_agg;
...
static void
abt_xlog_fix_aggregation_value(XLogReaderState *record)
{
	XLogRecPtr	lsn = record->EndRecPtr;
	xl_abtree_fix_aggregation_value *xlrec =
		(xl_abtree_fix_aggregation_value *) XLogRecGetData(record);
	Buffer		buf;
	
	/* XXX BLK_RESTORED may be required later if we allow concurrent? */
	if (XLogReadBufferForRedo(record, 0, &buf) == BLK_NEEDS_REDO)
	{
		...
		agg_ptr	= (char *) page + xlrec->offset;
		memcpy(agg_ptr, new_value, datalen);
...
abt_redo_agg_op(XLogReaderState *record)
{
	...
	{
		case XLOG_ABTREE_AGG_OP_FIX_AGG:
			abt_xlog_fix_aggregation_value(record);
			break;
		default:
...
{
	min_agg_info->extra_space_when_tid_missing =
		(int16) agg_info->extra_space_when_tid_missing;
	min_agg_info->extra_space_when_tid_present =
		(int16) agg_info->extra_space_when_tid_present;
