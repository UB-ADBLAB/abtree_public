...
static void show_index_sample_scan_details(uint64 sample_size,
			...
			ExplainState *es)
{
		...
		case T_IndexOnlyScan:
			{
				...
				{
					pname = sname = "Index Only Sample Scan";
				}
			}
			break;
	...
	switch (nodeTag(plan))
	{
		case T_IndexScan:
			...
			if (plan->qual)
				show_instrumentation_count("Rows Removed by Filter", 1,
										   planstate, es);
			if (((IndexScan *) plan)->is_sample_scan)
				show_index_sample_scan_details(
					((IndexScan *) plan)->sample_size,
					((IndexScan *) plan)->repeatable_expr,
		...
		case T_IndexOnlyScan:
			...
			if (((IndexOnlyScan *) plan)->is_sample_scan)
				show_index_sample_scan_details(
					((IndexOnlyScan *) plan)->sample_size,
					((IndexOnlyScan *) plan)->repeatable_expr,
					planstate, ancestors, es);
			break;
		case T_BitmapIndexScan:
...
static void
show_index_sample_scan_details(uint64 sample_size,
							   Expr *repeatable_expr,
							   PlanState *planstate,
							   List *ancestors,
							   ExplainState *es)
{
	...
	if (repeatable_expr)
	{
		...
		context = set_deparse_context_plan(es->deparse_cxt, planstate->plan,
										   ancestors);
		useprefix = list_length(es->rtable) > 1;
		repeatable = deparse_expression((Node *) repeatable_expr,
										context,
										useprefix,
										false);
	}
	...
	{
		ExplainIndentText(es);
		appendStringInfo(es->str, "Sampling: swr ("UINT64_FORMAT")",
						 sample_size);
		if (repeatable)
			appendStringInfo(es->str, " REPEATABLE (%s)", repeatable);
		appendStringInfoChar(es->str, '\n');
	...
	else
	{
		char buf[22];
		List *params = NIL;
		snprintf(buf, 22, UINT64_FORMAT, sample_size);
		params = lappend(params, buf);
