 ...
 *		IndexOnlyNextSample			sample next tuple
...
static TupleTableSlot *IndexOnlyNextSample(IndexOnlyScanState *node);
 ...
 *		IndexOnlyNextSample
 *
 *		Sample a tuple from the IndexOnlyScan node's index using
 *		amsampletuple() interface.
 * ----------------------------------------------------------------
 */
static TupleTableSlot *
IndexOnlyNextSample(IndexOnlyScanState *node)
{
	EState	   *estate;
	ExprContext *econtext;
	IndexScanDesc scandesc;
	...
	if (index_sample_scan_no_more_samples(&node->ioss_IndexSampleScanState))
	{
		/* We've returned enough samples for this scan. */
		return NULL;	
	}
	...
	/*
	 * OK, now that we have what we need, sample the next tuple.
	 */
	for (;;)
	{
		...
		random_number = index_sample_scan_next_random_number(
			&node->ioss_IndexSampleScanState);
		tid = index_samplenext_tid(scandesc, random_number);

		/* Warn the user if the rejection rate is too high. */
		index_sample_scan_check_for_high_rejection_rate(
			&node->ioss_IndexSampleScanState);

		CHECK_FOR_INTERRUPTS();

		/* We need to retry because this sample was rejected. */
		if (tid == NULL)
			continue;
		...
		/* Now we're sure we have a new sample. Decrement the counter. */
		index_sample_scan_got_new_sample(&node->ioss_IndexSampleScanState);
		return slot;
	}
...
ExecIndexOnlyScan(PlanState *pstate)
{
	...
	if (node->ioss_NumRuntimeKeys != 0 && !node->ioss_RuntimeKeysReady)
	{
		index_sample_scan_mark_rescan_for_runtime_keys_only(
			&node->ioss_IndexSampleScanState);
		ExecReScan((PlanState *) node);
	}

	if (((IndexOnlyScan *) node->ss.ps.plan)->is_sample_scan)
		return ExecScan(&node->ss,
						(ExecScanAccessMtd) IndexOnlyNextSample,
 ...
 *
 *		Also re-initialize the sampler if this is an index sample scan
 *		and we are not rescanning for the runtime keys only.
 * ----------------------------------------------------------------
 */
void
ExecReScanIndexOnlyScan(IndexOnlyScanState *node)
{
	...
	/*
	 * We may also need to restart the sample scan state.
	 * index_sample_scan_start() will inspect the isss_RescanForRuntimeKeysOnly
	 * bit to determine whether to restart.
	 */
	if (((IndexOnlyScan *) node->ss.ps.plan)->is_sample_scan)
	{
		index_sample_scan_start(
			&node->ioss_IndexSampleScanState,
...
ExecInitIndexOnlyScan(IndexOnlyScan *node, EState *estate, int eflags)
{
	...
	if (node->is_sample_scan)
	{
		index_sample_scan_init(&indexstate->ioss_IndexSampleScanState,
							   node->repeatable_expr,
							   (PlanState *) indexstate,
							   node->sample_size,
							   node->sample_size_expr);
		index_sample_scan_start(&indexstate->ioss_IndexSampleScanState,
