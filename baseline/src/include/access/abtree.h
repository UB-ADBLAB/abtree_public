 ...
 *
 *  t_tid | t_info | key values | INCLUDE columns, if any | [agg value]
 *
 ...
 * Pivot tuple format:
 *
 *  t_tid | t_info | key values | agg value | [heap TID]
 *
...
typedef struct ABTStackData
{
	...
	/* 
	 * abts_buf is set to the buffer containing the page if the page is pinned
	 * and locked, which can happen when we x-lock the path in the baseline.
	 * Otherwise, it's set to InvalidBuffer.
	 */
	Buffer		abts_buf;
...
typedef struct ABTDedupStateData
{
	...
	bool		save_agg;		/* Need to save the aggregation values? */
 ...
 * given the maxpostingsize and considering the aggregation value's size.
 */
#define ABTDedupMaxNumberOfTIDs(state, agg_info)	\
	(state->maxpostingsize / (sizeof(ItemPointerData) + agg_info->agg_stride)) \
	+ 1
 ...
 */
extern bool _abt_dedup_one_page(Relation rel, Buffer buf, Relation heapRel,
...
extern Buffer _abt_getroot(Relation rel, int access, bool xlock,
						   bool reject_non_fastroot);
 ...
 */
extern ABTStack _abt_search(Relation rel, ABTScanInsert key, Buffer *bufP,
						  int access, Snapshot snapshot, bool xlock_path,
						  bool reject_non_fastroot);
 ...
 */
extern ABTScanInsert _abt_mkscankey(Relation rel, IndexTuple itup);
extern void _abt_freestack(Relation rel, ABTStack stack);
extern void _abt_relbuf_stack(Relation rel, ABTStack stack);
