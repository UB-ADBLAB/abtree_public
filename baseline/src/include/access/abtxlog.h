...
{
	int16		extra_space_when_tid_missing;
	int16		extra_space_when_tid_present;
	int16		agg_typlen;
	int16		agg_stride;
} xl_abtree_minimal_agg_info;

#define SizeOfABtreeMinimalAggInfo \
	(offsetof(xl_abtree_minimal_agg_info, agg_stride) \
	 + sizeof(int16))
...
/* Currently we only allow up to 16 different agg redo ops. */
#define XLOG_ABTREE_AGG_OP_NUMBER_MASK	0xf
#define XLOG_ABTREE_AGG_OP_MAX_NUMBER_OF_OPS \
	(XLOG_ABTREE_AGG_OP_NUMBER_MASK + 1)
...
#define ABTGetXLogAggOpNumber(tag) \
	((tag).info & XLOG_ABTREE_AGG_OP_NUMBER_MASK)
#define ABTSetXLogAggOpNumber(tag, opno) \
	((tag).info = ((tag).info & (~XLOG_ABTREE_AGG_OP_NUMBER_MASK)) | opno)

/*
 * Backup Blk 0: the new value bytes.
 */
typedef struct xl_abtree_fix_aggregation_value
{
	xl_abtree_agg_op tag;
	int16		offset;			/* The offset to the aggregation in the page */
} xl_abtree_fix_aggregation_value;

#define SizeOfABTreeFixAggregationValue \
	(offsetof(xl_abtree_fix_aggregation_value, offset) + sizeof(int16))
