...
public class InsertManyRandom extends QueryBase {
	public InsertManyRandom(String url, boolean use_si, Meter meter,
							int counter_id) {
		m_meter = meter;
		...
		try {
			...
			for (i = 1; n == -1 || i <= n; ++i) {
				...
				if (insert_stmt.executeUpdate() != 1) {
					System.out.printf("failed to insert (%d, %d)\n", x, y);
				}
    ...
    public static void main(String[] args) throws IOException {
        if (args.length < 4) {
            System.out.println("usage: test.InsertManyRandom <dbname> <n> " +
		...
		if (args.length > 4) {
			...
			meter_file = String.format(
				"InsertManyRandom_%04d%02d%02d-%02d%02d%02d.meter",
		...
		if (num_threads == 1) {
			...
			InsertManyRandom instance = new InsertManyRandom(url, false, meter,
															 0);
	...
	public static class QueryThread extends Thread {
		...
		public void run() {
			m_instance = new InsertManyRandom(m_url, m_use_si, m_meter,
											  m_counter_id);
		...
		public InsertManyRandom getInstance() {
			return m_instance;
		}
		...
		private InsertManyRandom m_instance;
	}
