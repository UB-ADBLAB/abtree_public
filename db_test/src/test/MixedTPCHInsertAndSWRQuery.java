...
public class MixedTPCHInsertAndSWRQuery {
	public static void main(String args[]) throws IOException {
		int args_base = 0;
		if (args.length >= 1 && args[0].equals("-i")) {
			SWRTPCHQuery.ask_for_samples_attempted = true;
			args_base = 1;
		}

		if (args.length - args_base < 9) {
			System.out.println(
				"usage: test.MixedTPCHInsertAndSWRQuery [-i] <dbname> " +
				"<ninserts> <ninsert_threads> <seed> <insert_init_sleep_ms> " +
				"<nqueries> <sample_size> <nquery_threads> <query_init_sleep_ms> [meterfile]");
				System.exit(1);
		}
		...
		if (args.length - args_base > 9) {
			...
			meter_file = String.format(
				"MixedTPCHInsertAndSWRQuery_%04d%02d%02d-%02d%02d%02d.meter",
		...
		if (SWRTPCHQuery.ask_for_samples_attempted) {
			nattempt_counter = meter.add_counter("#attempts");
		}

		System.out.printf("%d %d %d\n", samples_counter, inserts_counter, nattempt_counter);
		...
		for (int i = 0; i < total_num_threads; ++i) {
			...
			if (i < nquery_threads) {
				threads[i] = new SWRTPCHQuery.QueryThread(
		...
		for (int i = 0; i < total_num_threads; ++i) {
			long duration;
			if (i < nquery_threads) {
				duration = ((SWRTPCHQuery.QueryThread) threads[i])
